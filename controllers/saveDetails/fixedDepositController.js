            ...
            db.query(
                ...
                (error, result) => {
                    ...
                    db.query('SELECT AccountNumber FROM ledgerdetails WHERE AccountNumber = ?', [AccountNumber], (error, result) => {
                        ...
                        db.query(
                            'INSERT INTO ledgerdetails (OpenDate, Period, AccountBalance, PaidFDInterestAmount, CustomerID, AccountType, InterestRate, ledgerID, AccountNumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                            [OpenDate, Period, AccountBalance, PaidFDInterestAmount, CustomerID, AccountType, InterestRate, ledgerID, AccountNumber],
                            (error, result) => {
                                if (error) return res.status(500).json({ message: 'Server error, please try again later' });
